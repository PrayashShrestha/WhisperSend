(() => {
    ...
    const SpeechmasticsTranscriber = (() => {
        ...
        function flushPendingAudio() {
            ...
            // OPTIMIZATION: WebSocket backpressure monitoring - prevents data loss under network pressure
            // Check WebSocket bufferedAmount for backpressure (prevents buffer overflow)
            const MAX_BUFFERED = 64 * 1024; // 64KB threshold - adjust based on network conditions

            while (pendingAudioQueue.length > 0) {
                // Check if WebSocket send buffer is getting full
                if (ws.bufferedAmount > MAX_BUFFERED) {
                    // Backpressure detected - keep remaining buffers in queue
                    // They will be sent on next flush attempt
                    log(`WebSocket backpressure: ${ws.bufferedAmount} bytes buffered, ${pendingAudioQueue.length} chunks queued`);
                    break;
                }

                const buffer = pendingAudioQueue.shift();
                pendingAudioBytes -= buffer.byteLength;
                ...
                try {
                    ...
                    // Re-queue failed buffer at front to preserve order
                    pendingAudioQueue.unshift(buffer);
                    pendingAudioBytes += buffer.byteLength;
                    break;
                }
            }
        ...
        function handleMessage(data) {
            // OPTIMIZATION: Fast path for partial transcripts (80% of messages)
            // Skip full JSON.parse and use regex extraction (5-10x faster)
            if (data.includes('"message":"AddPartialTranscript"')) {
                try {
                    // Extract transcript text using regex (much faster than JSON.parse)
                    const transcriptMatch = /"transcript":"([^"\\]*(?:\\.[^"\\]*)*)"/.exec(data);
                    if (transcriptMatch) {
                        const transcript = transcriptMatch[1].replace(/\\(.)/g, '$1'); // Unescape
                        if (transcript && transcript.trim()) {
                            lastSpeechTime = Date.now();
                            currentPartial = transcript;
                            notifyPartialTranscription(transcript);
                            return; // Skip full parse - saves 2-3ms per message
                        }
                    }
                } catch (err) {
                    // Fall through to full parse on error
                    log("Fast path parsing failed, falling back to full parse:", err);
                }
            }

            // FULL PARSE: For all other messages or if fast path fails
            try {
